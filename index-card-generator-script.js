...
function setCardId(backlogItem, card) {
    card.getCell(3, 5).setValue(backlogItem['Id']);
}
...
function setSlogan(backlogItem, card) {
    card.getCell(5, 3).setValue(backlogItem['Slogan']);
}

function setInfo(backlogItem, card) {
    card.getCell(7, 3).setValue(backlogItem['Info']);
}

function setEstimate(backlogItem, card) {
    card.getCell(5, 5).setValue(backlogItem['Estimate']);
}
...
function createCards(backlogItems) {
	...
	setSlogan(backlogItems[i], card);
	setInfo(backlogItems[i], card);
    ...
    Browser.msgBox("Generation Complete!");
}
